...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    $record = '; :local mac $"mac-address"; :local date [/system clock get date]; :local year [:pick $date 0 4]; :local month [:pick $date 5 7]; :local time [/system clock get time]; :local now ($date . "T" . $time . ";" . $address . ";" . $mac); :local record "$date-|-$time-|-$user-|-'.$price.'-|-$address-|-$mac-|-'.$validity.'-|-'.$name.'-|-$comment"; :local saleId [/system script find where comment="mikhmon" and name~("-|-" . $user . "-|-") and name~("-|-" . $comment . "$")]; :if ([:len $saleId] = 0) do={ /system script add name=$record owner="$month$year" source="$date" comment="mikhmon"; :set saleId [/system script find where comment="mikhmon" and name=$record]; }; :local loginName [/system script get $saleId name]; :local loginId [/system script find where comment="login" and name=$loginName]; :if ([:len $loginId] = 0) do={ /system script add name=$loginName owner="" source="$now" comment="login"; } else={ /system script set $loginId source=([/system script get $loginId source] . "|" . $now); }';
