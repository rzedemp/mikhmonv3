...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    $record = '; :local record ("$date-|-$time-|-$user-|-'.$price.'-|-$address-|-$mac-|-'.$validity.'-|-'.$name.'-|-$userComment"); :local saleId [/system script find where comment="mikhmon" and name~$user and name~$userComment]; :if ([:len $saleId] = 0) do={ /system script add name=$record owner="$month$year" source="$date" comment="mikhmon"; }; /system script add name=$user owner="" source=$record comment="mikhmon login";';
    
    $onlogin = ':put (",'.$expmode.',' . $price . ',' . $validity . ','.$sprice.',,' . $getlock . ',"); { :local mac $"mac-address"; :local date [/system clock get date]; :local year [:pick $date 0 4]; :local month [:pick $date 5 7]; :local time [/system clock get time]; :log info ("$date-|-$time-|-$user-|-'.$price.'-|-$address-|-$mac-|-'.$validity.'-|-'.$name.'"); :local userComment [/ip hotspot user get [/ip hotspot user find where name="$user"] comment]; :local ucode [:pick $userComment 0 2]; :if ($ucode = "vc" or $ucode = "up" or $userComment = "") do={ /system scheduler add name="$user" disabled=no start-date=$date interval="'.$validity.'"; :delay 5s; :local exp [/system scheduler get [/system scheduler find where name="$user"] next-run]; :log info ("$user: expired on $exp"); :local getxp [:len $exp]; :if ($getxp = 14) do={ :local d [:pick $exp 0 5]; :local t [:pick $exp 6 15]; :local exp ("$year-$d $t"); /ip hotspot user set comment="$exp" [find where name="$user"]; }; :if ($getxp = 8) do={ /ip hotspot user set comment="$date $exp" [find where name="$user"]; }; :if ($getxp > 14) do={ /ip hotspot user set comment="$exp" [find where name="$user"]; }; :delay 5s; /system scheduler remove [find where name="$user"]; :log info ("$user: delete scheduler")';
