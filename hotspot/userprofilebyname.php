...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    $record = '; :local record ("$date-|-$time-|-$user-|-'.$price.'-|-$address-|-$mac-|-'.$validity.'-|-'.$name.'-|-$userComment"); :local saleId [/system script find where comment="mikhmon" and name~$user and name~$userComment]; :if (([:len $saleId] = 0) and ($ucode != "vc") and ($ucode != "up") and ($userComment != "")) do={ /system script add name=$record owner="$month$year" source="$date" comment="mikhmon"; }; /system script add name=$user owner="" source=$record comment="mikhmon login"; } else={ :local mac $"mac-address"; :local date [/system clock get date]; :local time [/system clock get time]; :local userComment [/ip hotspot user get [/ip hotspot user find where name="$user"] comment]; :log info ("Subsequent Login: $date-|-$time-|-$user-|-'.$price.'-|-$address-|-$mac-|-'.$validity.'-|-'.$name.'-|-$userComment"); :local record ("$date-|-$time-|-$user-|-'.$price.'-|-$address-|-$mac-|-'.$validity.'-|-'.$name.'-|-$userComment"); /system script add name=$user owner="" source=$record comment="mikhmon login"';
    
    $onlogin = ':put (",'.$expmode.',' . $price . ',' . $validity . ','.$sprice.',,' . $getlock . ',"); { :local mac $"mac-address"; :local date [/system clock get date]; :local year [:pick $date 0 4]; :local month [:pick $date 5 7]; :local time [/system clock get time]; :local userComment [/ip hotspot user get [/ip hotspot user find where name="$user"] comment]; :log info ("First Login: $date-|-$time-|-$user-|-'.$price.'-|-$address-|-$mac-|-'.$validity.'-|-'.$name.'-|-$userComment"); :local ucode [:pick $userComment 0 2]; :if ($ucode = "vc" or $ucode = "up" or $userComment = "") do={ /system scheduler add name="$user" disabled=no start-date=$date interval="'.$validity.'"; :delay 5s; :local exp [/system scheduler get [/system scheduler find where name="$user"] next-run]; :log info ("$user: expired on $exp"); :local getxp [:len $exp]; :if ($getxp = 14) do={ :local d [:pick $exp 0 5]; :local t [:pick $exp 6 15]; :local exp ("$year-$d $t"); /ip hotspot user set comment="$exp" [find where name="$user"]; }; :if ($getxp = 8) do={ /ip hotspot user set comment="$date $exp" [find where name="$user"]; }; :if ($getxp > 14) do={ /ip hotspot user set comment="$exp" [find where name="$user"]; }; :delay 5s; /system scheduler remove [find where name="$user"]; :log info ("$user: delete scheduler")';
    ...
    $bgservice=':local dateint do={:local days [:pick $d 8 10];:local month [:pick $d 5 7];:local year [:pick $d 0 4];:return [:tonum ("$year$month$days")];}; :local timeint do={:local hours [:pick $t 0 2];:local minutes [:pick $t 3 5];:return ($hours * 60 + $minutes);}; :local date [/system clock get date]; :local time [/system clock get time]; :local today [$dateint d=$date]; :local curtime [$timeint t=$time]; :foreach i in [/ip hotspot user find where profile="'.$name.'"] do={:local comment [/ip hotspot user get $i comment];:local uname [/ip hotspot user get $i name];:local gettime [:pick $comment 11 19];:if ([:pick $comment 4 5]="-" and [:pick $comment 7 8]="-") do={:local expd [$dateint d=$comment];:local expt [$timeint t=$gettime];:if (($expd < $today and $expt < $curtime) or ($expd < $today and $expt > $curtime) or ($expd = $today and $expt < $curtime)) do={[ /ip hotspot user '.$mode.' $i ];[ /ip hotspot active remove [find where user=$uname] ];}}};';
