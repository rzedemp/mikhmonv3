...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    $record = ' ; :local saleId [/system script find where comment="mikhmon" and name~$user and name~$exp]; :if (([:len $saleId] = 0)) do={ /system script add name="$date-|-$time-|-$user-|-'.$price.'-|-$address-|-$mac-|-'.$validity.'-|-'.$name.'-|-$exp" owner="$month$year" source="$date" comment="mikhmon"; } ';

    $onlogin = ':put (",'.$expmode.',' . $price . ',' . $validity . ','.$sprice.',,' . $getlock . ',"); { :local mac $"mac-address"; :local date [/system clock get date]; :local year [:pick $date 0 4]; :local month [:pick $date 5 7]; :local time [/system clock get time]; :local userComment [/ip hotspot user get [/ip hotspot user find where name="$user"] comment]; :local record ("$date-|-$time-|-$user-|-'.$price.'-|-$address-|-$mac-|-'.$validity.'-|-'.$name.'-|-$userComment"); :log info ("On Login: $record"); :local ucode [:pick $userComment 0 2]; /system script add name="$date-|-$time-|-$user" owner=$user source=$record comment="mikhmon login"; :if ($ucode = "vc" or $ucode = "up" or $userComment = "") do={ :log info ("On Login 2: $record"); /system scheduler add name="$user" disabled=no start-date=$date interval="'.$validity.'"; :delay 5s; :local exp [/system scheduler get [/system scheduler find where name="$user"] next-run]; :log info ("$user: expired on $exp"); :local getxp [:len $exp]; :if ($getxp = 14) do={ :local d [:pick $exp 0 5]; :local t [:pick $exp 6 15]; :set exp "$year-$d $t"; /ip hotspot user set comment="$exp" [find where name="$user"]; }; :if ($getxp = 8) do={ :set exp "$date $exp"; /ip hotspot user set comment="$exp" [find where name="$user"]; }; :if ($getxp > 14) do={ /ip hotspot user set comment="$exp" [find where name="$user"]; }; :delay 5s; /system scheduler remove [find where name="$user"]; :log info ("$user: delete scheduler") ';
